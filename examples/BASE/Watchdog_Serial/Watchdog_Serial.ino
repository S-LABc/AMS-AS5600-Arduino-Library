 ...
 * Watchdog_Serial
 * 
 * Демонстрация включения и отключения сторожевого 
 * таймера у датчика AS5600
 * 
 * Подключение:
 * AS5600   Board
 * VCC   -> +3V3
...
void loop() {
  if(Sensor.isWatchdog()) {
    Serial.println("Сторожевой таймер Включен. ВЫКЛЮЧЕНИЕ...");
    Sensor.disableWatchdog(); // bool disableWatchdogVerify()
  } else {
    Serial.println("Сторожевой таймер Выключен. ВКЛЮЧЕНИЕ...");
    Sensor.enableWatchdog(); // bool enableWatchdogVerify()
  }
